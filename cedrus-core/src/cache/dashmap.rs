...
impl Cache for DashMapCache {
    ...
    async fn project_set_entities(
        ...
        for entity in entities {
            self.entities
                .insert((*project_id, entity.uid().clone()), entity.clone());
        }
        Ok(())
    }
    ...
    async fn project_set_policies(
        ...
        for (policy_id, policy) in policies {
            self.policies
                .insert((*project_id, policy_id.clone()), policy.clone());
        }
        Ok(())
    }
    ...
    async fn project_set_templates(
        ...
        for (policy_id, template) in templates {
            self.templates
                .insert((*project_id, policy_id.clone()), template.clone());
        }
        Ok(())
    }
    ...
    async fn project_set_template_links(
        ...
        for link in template_links {
            self.template_links
                .insert((*project_id, link.new_id.clone()), link.clone());
        }
        Ok(())
    }
    ...
    async fn project_del_template_links(
        ...
        for policy_id in policy_ids {
            self.template_links
                .remove(&(*project_id, policy_id.clone()));
        }
        Ok(())
    }
